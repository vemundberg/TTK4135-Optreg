...
A1 = [1 delta_t 0 0;0 1 -K_2*delta_t 0; 0 0 1 1*delta_t;0 0 -K_1*K_pp*delta_t 1-K_1*K_pd*delta_t];
...
u_opt = timeseries(u,t); % Making time series for Simulink
...
opt = [u_opt;x1_opt;x2_opt;x3_opt;x4_opt];
...
hold on;
plot(t,p);
grid on, xlabel('t[s]'), ylabel('Pitch [rad]'), legend('p*','p'), title('p* and p with feedback');
...
tr_star = travel.ans(2,:);
tr = travel.ans(3,:);

plot(t,tr_star);
hold on;
plot(t,tr);
grid on, xlabel('t[s]'), ylabel('Travel [rad]'), legend('\lambda *','\lambda'), title('\lambda * and \lambda with feedback');